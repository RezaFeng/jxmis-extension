...
- 经营分析：在 JXPMO 项目模块左侧菜单增加“经营分析”；打开面板时实时获取组织、项目和必要的当前周期日报，确定有效部门后，再按部门和日期生成 35 项核心值、项目/PM/预算/周期执行/里程碑/回款明细、风险诊断和全部部门总览。
- 相邻周期比较：首次正式查询时按筛选周期获取紧邻上期日报，并用本期有日报投入的同一正式项目集合计算本期、上期、变化和环比。
...
3. 打开 JXPMO 项目模块，在左侧项目菜单中点击“经营分析”。面板会先并行加载组织、项目和必要的当前周期日报；全部成功前部门和查询控件保持禁用，失败时刷新业务页面重新加载。
4. 初始化完成后选择部门、开始日期和结束日期，点击“查询”。默认日期为最近一个完整自然周，零有效项目部门不会出现在下拉框中。
5. 默认正式范围要求项目 `realExeuCost > 0` 且本期日报投入大于 0。筛选条件未变化时，点击查询或切换部门不会重新获取项目、当前周期日报和已经取得的上一周期日报；首次正式查询再按需获取上一周期日报和项目明细。
6. 部分来源失败时查看完整性诊断并使用“仅重试失败项”；失败上下文和面板工作数据只保留在当前页面内存中。
7. 点击“导出HTML”生成当前正式全量口径的离线报告。项目表中的临时筛选或勾选不会改变顶部正式统计和导出。

Options 页还可配置 AI、项目经理覆盖、经营项目筛选和预警阈值。Options 修改后需要刷新业务页面，使新配置参与初始化。经营分析不保存查询缓存、报告快照或指标历史；刷新页面后会重新读取实时数据。
...
截至 2026-07-19，构建、205 个 Node 单元/集成测试和 9 个离线 Chromium E2E 已通过。E2E 覆盖原三类自动化、Options、项目菜单、面板实时初始化、默认投入范围、工作集复用、空值、合同金额兜底、里程碑确认状态、上期比较、分页、全部部门、内存重试、取消、会话失效、HTML 下载和无经营 IndexedDB，并断言不请求真实 `jxmis.cyberwing.cn`。
