...
- [ ] `npm run verify` 全部通过，原三类自动化无回归。
- [ ] 查询和刷新每次都请求实时数据，不发送经营持久化消息。
- [ ] 扩展升级后旧 `cw-business-analytics` IndexedDB 被幂等清理，运行时不再创建。
- [ ] 默认正式范围仅包含本期日报投入大于 0 的项目，并在页面显示 `N/M`。
- [ ] 成功空数值和零分母显示 0；技术失败显示“未获取”。
- [ ] 累计经营指标只显示实时值，不出现伪历史环比。
- [ ] 期间指标使用同一本期正式项目集合显示本期、紧邻上期和环比。
- [ ] 全部部门通过一次实时采集生成，公共来源不按部门重复请求。
- [ ] 页面不出现快照、缓存、区间执行报告或“无法历史回溯”文案。
- [ ] 临时筛选不能覆盖正式统计，导出始终使用正式实时报告。
- [ ] HTML 离线可用且不包含认证、人员级日报、失败 descriptor 或外部请求。
- [ ] README、架构、数据字典和 UAT 与实时实现一致。
